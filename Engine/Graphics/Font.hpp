...
public:
  ...
  Font(std::string name, span<S<const Texture2>> textures, std::vector<Glyph>&& glyphs);
  Font(std::string name, span<S<const Texture2>> textures, span<Glyph> glyphs);
  Font() = default;
  ...
  const Texture2* texture(uint index = 0) const { return mTextures[index].get(); }
  static inline S<const Font>& Default() { return sDefaultFont; };
protected:
  std::vector<Glyph> mGlyphs;
  std::vector<Face> mFaces;
  std::vector<S<const Texture2>> mTextures;
