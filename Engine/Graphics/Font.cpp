...
Font::Font(std::string name, span<S<const Texture2>> textures, std::vector<Glyph>&& glyphs)
  : name(std::move(name))
  , mGlyphs(glyphs) {
...
//    vec2 gsize = g.uv.size() / size;
  ...
  for(auto& texpath: textures) {
    ...
    auto tex = Resource<Texture2>::get(p);
    if(!tex) {
      Resource<Texture2>::define(p);
      tex = Resource<Texture2>::get(p);
    }

    font->mTextures.push_back(tex);
  }
