...
void FrameBuffer::setColorTarget(const Texture2::scptr_t& tex, uint index) {
...
void FrameBuffer::defineColorTarget(const Texture2::scptr_t tex, uint index, bool allowUav) {
  mDesc.defineColorTarget(index, tex->format(), allowUav);
  setColorTarget(tex, index);
}

void FrameBuffer::defineDepthStencilTarget(const Texture2::scptr_t tex, bool allowUav) {
  mDesc.defineDepthTarget(tex->format(), allowUav);
  setDepthStencilTarget(tex);
}

void FrameBuffer::setDepthStencilTarget(Texture2::scptr_t tex) {
