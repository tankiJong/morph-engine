...
void Console::render() const {
  ...
  mRenderer->drawAABB2({ screenBounds.mins, 
                       ...
                       Rgba(0, 0, 0, 200));
  // log
  mRenderer->drawAABB2({ screenBounds.mins + vec2{ 0, inputBoxHeight}, 
                         screenBounds.maxs }, 
                       Rgba(0, 50, 70, 200));

  BitmapFont* font = BitmapFont::getDefaultFont();
  EXPECTS(font != nullptr);
  float chWidth = font->getCharWidth(FONT_SIZE, 'A', FONT_ASPECT);

  // ###### draw selection
  EXPECTS(mSelection.min >= 0 && mSelection.max >=0);
  if(mSelection.size() > 0) {
    vec2 selectionBottomLeft = screenBounds.mins + vec2{ mSelection.min * chWidth + WORD_PADDING, 0 };
...
//  consumeConsoleText(inputText, prepareTextRenderData);
  mRenderer->drawText2D(screenBounds.mins + vec2{ WORD_PADDING, WORD_PADDING }, 
                        mInputStream, FONT_SIZE, DEFAULT_TEXT_COLOR, FONT_ASPECT);
  ...
  for (auto it = mLogStream.rbegin() + mLineNumberAtBottom; it != mLogStream.rend(); ++it) {
    ...
    mRenderer->drawText2D({ 0, startY }, stringChunks, FONT_SIZE, colors, nullptr, FONT_ASPECT);
  ...
  mRenderer->drawAABB2({ scrollBarMins,
                         scrollBarMaxs },
  ...
  mRenderer->drawAABB2({ scrollBarMins + vec2{ 0, barPadding},
                         scrollBarMins + vec2{ SCROLLBAR_WIDTH, barHeight + barPadding } },
                       Rgba(255, 255, 255, 150));
  // ###### draw cursor
  if(mDrawCursor) {
    vec2 cursorBottomPos(mCursorPosition * chWidth + WORD_PADDING, WORD_PADDING);
    vec2 cursorTopPos = cursorBottomPos + vec2(0, FONT_SIZE);

    mRenderer->drawLine(vec3(cursorBottomPos), vec3(cursorTopPos), Rgba::white, Rgba::white, 3);
  }
}
